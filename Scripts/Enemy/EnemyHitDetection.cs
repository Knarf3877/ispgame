...
public class EnemyHitDetection : MonoBehaviour
{
    ...
    public ParticleSystem playerHitFX;
    ...
    void Update()
    {
        ...
        if (Time.time > destructionTime)
            DestroyBullet(transform.position, false, false);
    }
    void FixedUpdate()
    {
        if (isFired == false)
    ...
    public void MoveBullet(float damage)
    {
        ...
        if (rayHasHit == true)
        {
            ...
            if (target)
            {
                ...
                DestroyBullet(rayHit.point, true, true);
            }

            DestroyBullet(rayHit.point, true, false);
        }
        else
        {
    ...
    public void DestroyBullet(Vector3 position, bool fromImpact, bool isPlayer)
    {
        if (fromImpact == true && explodeFX != null)
        {
            ...
            if (isPlayer)
            {
                ParticleSystem tempExplodeFX = Instantiate(playerHitFX, transform.position, transform.rotation);
